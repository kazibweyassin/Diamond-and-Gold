...
const compactDefaultSections = [
	...
	},
	{
		label: 'Connect',
		links: [
			{ name: 'WhatsApp', href: 'https://wa.me/256704833021', external: true },
	...
	return (
		<footer className="bg-[var(--color-green-deep)] border-t border-[var(--color-green-dark)]/40 px-6 py-16 text-[var(--color-white)]">
			<div className="max-w-7xl mx-auto">
				<div className={variant === 'compact' ? 'grid gap-12 md:grid-cols-[2fr_1fr_1fr_1fr_1fr] mb-12' : 'grid gap-12 md:grid-cols-[2fr_1fr_1fr_1fr] mb-12'}>
					<div>
						{showBrandImage && (
							<Image
								src="/Logo.png"
					...
					{resolvedSections.map((section) => (
						<div key={section.label}>
							<p className="text-[10px] uppercase tracking-[0.25em] text-white/85 mb-5">
								{section.label}
							</p>
							...
							<div className="flex flex-col gap-3 text-sm">
								{section.links.map((link) => (
									<Link
										...
										className="text-[var(--color-green-tint)] hover:text-[var(--color-green-bright)] transition"
									>
										{link.name}
									</Link>
								))}
				...
				<div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-t border-[var(--color-green-dark)]/40 pt-6 text-[11px] text-white/80">
					<span>{resolvedCopyright}</span>
					<div className="flex gap-6">
						<Link href="/privacy-policy" className="text-[var(--color-green-tint)] hover:text-[var(--color-green-bright)] transition">
							Privacy Policy
						</Link>
						<span>{resolvedBottomNote}</span>
					</div>
				</div>
			</div>
		</footer>
