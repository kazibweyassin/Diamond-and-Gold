...
function StatIcon({ kind }: { kind: 'shield' | 'clock' | 'file' | 'globe' }) {
  if (kind === 'shield') return <Shield size={18} strokeWidth={1.7} />;
  if (kind === 'clock') return <Clock size={18} strokeWidth={1.7} />;
  if (kind === 'file') return <FileText size={18} strokeWidth={1.7} />;
  return <Globe size={18} strokeWidth={1.7} />;
}
  ...
  return (
    <main className="font-dca-marketing" style={{ minHeight: '100vh', background: '#F7F6F2', color: '#0D0D0D', WebkitFontSmoothing: 'antialiased' }}>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 768px) {
          ...
          main.font-dca-marketing > hr.rule {
            margin: 0 1rem;
          }
          ...
          .hero-slide-row { align-items: flex-start; }
          ...
          .corp-grid   { grid-template-columns: 1fr !important; gap: 1.5rem !important; }
          .corp-actions { flex-direction: column; }
          .corp-actions a { width: 100%; justify-content: center; }
          .corp-stats   { grid-template-columns: repeat(2, 1fr) !important; }
        }
          ...
          <div className="hero-copy">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
              ...
              <div className="hero-metrics">
                {STATS.map(({ value, label, unit, icon }) => (
                  <div key={label} className="hero-stat">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span className="stat-unit" style={{ marginBottom: 0 }}>{unit}</span>
                      <span aria-hidden="true" style={{ color: 'rgba(232,201,106,0.95)', display: 'inline-flex' }}>
                        <StatIcon kind={icon as 'shield' | 'clock' | 'file' | 'globe'} />
                      </span>
                    </div>
          ...
          <div className="buyers-cols" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1.5rem' }}>
            {[
              { title: 'Every document, included',    body: 'Origin certificate, assay report, custody records, KYC package — all included with every shipment.',
                icon: <FileCheck size={36} color="var(--navy)" /> },
              { title: 'Lab-verified purity',          body: 'Independent ISO-certified laboratories verify every shipment. Stamped assay certificate ships with your order.',
                icon: <Beaker size={36} color="var(--navy)" /> },
              { title: 'Insured &amp; tracked delivery', body: 'Full-value shipment insurance. Real-time tracking. A named team member who answers your calls.',
                icon: <Truck size={36} color="var(--navy)" /> },
            ].map((item, i) => (
      ...
      <section style={{ background: 'linear-gradient(135deg, #0A1628 0%, #1a2d42 100%)', padding: '4rem 2rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        >
          <div className="corp-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', alignItems: 'center' }}>
            <div>
              <div style={{ fontFamily: 'var(--dca-mono)', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ display: 'block', width: 18, height: 1, background: 'var(--gold)' }} />
                ABOUT US
              </div>
              <h3 style={{ fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 300, color: '#fff', letterSpacing: '-0.025em', lineHeight: 1.15, marginBottom: '1rem' }}>
                Learn Our <strong style={{ fontWeight: 600 }}>Corporate Profile</strong>
              </h3>
              <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', lineHeight: 1.8, marginBottom: '1.5rem', fontWeight: 300 }}>
